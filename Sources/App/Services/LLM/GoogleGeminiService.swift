...
extension GoogleGeminiService: ServiceLifecycle {
        ...
        do {
            ...
            guard !config.geminiApiKey.isEmpty else {
                throw ConfigurationError.missingRequired(
                    key: "GEMINI_API_KEY",
                    suggestion: "Set GEMINI_API_KEY environment variable for LLM service"
                )
            }
            ...
            let response = try await app.client.get(
                URI(string: "https://generativelanguage.googleapis.com/v1beta/models"),
                headers: ["x-goog-api-key": config.geminiApiKey]
            )
            let elapsed = Date().timeIntervalSince(start)
            guard response.status == .ok else {
                let error = NSError(
                    domain: "GoogleGeminiService", code: Int(response.status.code),
                    userInfo: [
                        NSLocalizedDescriptionKey:
                            "API request failed with status: \(response.status)"
                    ])
                throw ServiceRegistryError.serviceInitializationFailed("Gemini API", error)
            }
            app.logger.info(
                "Gemini service started",
                metadata: [
                    "response_time_ms": .string(String(format: "%.2f", elapsed * 1000))
                ])
        } catch {
            app.logger.error(
                "Gemini service startup failed",
                metadata: [
    ...
    func shutdown(_ app: Application) async throws {
        app.logger.info("Gemini service shut down gracefully")
    }
}

// MARK: - ServiceHealthCheck

extension GoogleGeminiService: ServiceHealthCheck {
    func isHealthy() async -> Bool {
        do {
            ...
            guard elapsed < 3.0 else { return false }
            return true
        } catch {
            app.logger.warning(
    ...
    func healthCheckName() -> String { "Google Gemini LLM Service" }
}
